...
/**
 * tools_call_benchmark
 *
 * Mesure la latence des tools/call (get_mcp_client_ip) sur les servers témoins.
 * Conçu pour tourner en décalé par rapport à benchmark.js (tools/list)
 * afin de simuler une utilisation réelle : discover tools, puis appeler un tool.
 *
 * Usage:
 *   node tools_call_benchmark.js
 *   node tools_call_benchmark.js --group servers_temoins
 *   node tools_call_benchmark.js --timeout 10000
 *   node tools_call_benchmark.js --jitter 300
 */
...
// --- tools/call request ---
async function measure_tools_call(url) {
  ...
  try {
    const res = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        id: 1,
        method: "tools/call",
        params: { name: "get_mcp_client_ip", arguments: {} },
      }),
    ...
    try {
      ...
      tool_result = body?.result?.content?.[0]?.text ?? null;
    } catch (e) {
...
console.log(`\ntools_call_benchmark — tools/call → get_mcp_client_ip`);
...
for (const { group, name, url } of servers) {
  ...
  const result = await measure_tools_call(url);
  ...
  if (result.ok) {
    process.stdout.write(
      `\r  [${group}/${name}] ${roundtrip_ms}ms  ip_client=${result.tool_result}\n`
    );
  } else {
    process.stdout.write(
...
writeFileSync(filepath, JSON.stringify({
  benchmark: "tools/call",
  tool_name: "get_mcp_client_ip",
